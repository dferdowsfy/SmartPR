...
type BrandingBody = { company_name?: string; logo_url?: string; primary_color?: string };
...
export async function PUT(
  ...
  await pool.query(
    `INSERT INTO workspace_branding (workspace_id, company_name, logo_url, primary_color, updated_at)
     VALUES ($1,$2,$3,$4,now())
     ON CONFLICT (workspace_id) DO UPDATE SET
       ...
       updated_at = now()`,
    [workspaceId, companyName, logoUrl, primaryColor]
  );
  ...
  await auditLog({
    ...
    details: { company_name: companyName, logo_url: logoUrl, primary_color: primaryColor },
  });
