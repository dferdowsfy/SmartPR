...
export async function GET(req: Request) {
  ...
  // Never resolve local-only draft ids or malformed values: they are not
  // persisted businesses. (UUID validation also keeps the ::uuid casts
  // below from throwing — Postgres has no uuid = text operator, so the
  // params must be cast, not compared as text.)
  const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  if ((matterId && !UUID_RE.test(matterId)) || (businessId && !UUID_RE.test(businessId))) {
