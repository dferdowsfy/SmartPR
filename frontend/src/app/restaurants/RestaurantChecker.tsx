  ...
  return <div className={styles.shell} lang={language}>
    <header className={styles.header}>
      ...
      <div className={styles.languages} aria-label={t('Language', 'Idioma')}>
        {(['en', 'es'] as const).map(l => <button key={l} onClick={() => { setLanguage(l); window.history.replaceState(null, '', `${l === 'es' ? '/es/restaurantes' : '/restaurants'}${window.location.search}`); }} aria-pressed={language === l}>{l.toUpperCase()}</button>)}
      </div>
    </header>
