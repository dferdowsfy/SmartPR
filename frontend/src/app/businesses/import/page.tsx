      ...
      <main className="mx-auto max-w-5xl px-5 py-8">
        ...
        {step === 1 ? (
          <form onSubmit={preview} className="mt-6 rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-brand">Manage an existing business</p>
            ...
            <fieldset className="mt-6"><legend className="text-sm font-bold text-[#161616]">Relevant operating context</legend><p className="mt-1 text-xs text-slate-500">These facts are inputs to the rules engine. Select only what applies.</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{CONTEXT.map(([key, label]) => <label key={key} className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 text-sm text-slate-700"><input type="checkbox" checked={Boolean(answers[key])} onChange={(event) => setAnswers((current) => ({ ...current, [key]: event.target.checked }))} className="h-4 w-4 accent-brand" />{label}</label>)}</div></fieldset>
          ...
          <div className="mt-6 space-y-5">
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 text-brand" /><div><h1 className="text-xl font-bold text-[#161616]">Required obligations</h1><p className="mt-1 text-sm text-slate-500">Generated from the {knowledgeSource === "PUBLISHED_SNAPSHOT" ? "published regulatory graph" : "bundled deterministic knowledge base"}. AI cannot add obligations; it is used only to extract fields from evidence.</p></div></div>
              <div className="mt-4 grid gap-2 md:grid-cols-2">{obligations.map((obligation) => <label key={obligation.requirementId} className="flex items-start gap-3 rounded-xl border border-slate-200 p-3"><input type="checkbox" checked={unknown.has(obligation.requirementId)} onChange={(event) => setUnknown((current) => { const next = new Set(current); if (event.target.checked) next.add(obligation.requirementId); else next.delete(obligation.requirementId); return next; })} className="mt-1 h-4 w-4 accent-brand" /><span><span className="block text-sm font-semibold text-[#161616]">{obligation.name}</span><span className="block text-xs text-slate-500">{obligation.agency || "Agency not recorded"}</span><span className="mt-1 block text-[11px] text-slate-400">Check if the current status or expiration still needs to be determined.</span></span></label>)}</div>
            </section>

            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h2 className="text-lg font-bold text-[#161616]">Existing permits, licenses, and certificates</h2><p className="text-sm text-slate-500">Match each upload only to an obligation already generated by the rules engine.</p></div><label className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-xl bg-brand px-4 py-2.5 text-sm font-semibold text-white"><FileUp className="h-4 w-4" /> Upload documents<input type="file" multiple accept=".pdf,.txt,.png,.jpg,.jpeg" onChange={(event) => addFiles(event.target.files)} className="hidden" /></label></div>
