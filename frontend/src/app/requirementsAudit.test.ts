...
test("CASE Y: non-food personal-care business types no longer get the food-establishment health permit as REQUIRED", () => {
  // 2026-09-18 21:00 QA cycle (S47, Carolina): an existing beauty salon was
  // shown DOC_HEALTH_PERMIT (Health / Sanitary Permit — cited to Ley 81-1912,
  // "reglamentación sanitaria de establecimientos de alimentos", the FOOD
  // establishment sanitary permit) as verify_existing, and a new salon as
  // REQUIRED. Salons, barbershops and nail salons are not food
  // establishments; beauty establishments are currently unregulated in PR
  // (PS 971 would create a Dept.-of-Health operating license — proposed,
  // not enacted). Mirrors the 332b659 pattern (CASE P): the 2026-09-16
  // validated review and the 332b659 healthcare sweep missed these 6
  // personal-care siblings (beauty salon, barbershop, nail salon, spa,
  // massage therapy studio, esthetics studio); they now carry the same
  // honest needs_more_information posture. RULE_0664 lesson applied:
  // heuristic+NMI rules carry no compliance_mode. RULE_0160 (tattoo)
  // deliberately excluded — Dept. of Health regulates tattoo studios with
  // sanitation norms and a studio license (Ley 318-1999, Art. 15-17).
  const DOC_HEALTH = docByName("health", "sanitary");
  const personalCareBTs: Array<[string, string]> = [
    ["Beauty Salon", "RULE_0155"],
    ["Barbershop", "RULE_0156"],
    ["Nail Salon", "RULE_0157"],
    ["Spa", "RULE_0158"],
    ["Massage Therapy Studio", "RULE_0159"],
    ["Esthetics Studio", "RULE_0162"],
  ];
  for (const [bt, rule] of personalCareBTs) {
    const { classified } = classify(
      {
        municipalityName: "Carolina",
        businessTypeName: bt,
    ...
    // Existing businesses must also see NMI, not verify_existing —
    // verify_existing on a heuristic+NMI rule would mis-frame the posture.
    const existing = classify(
      {
        municipalityName: "Carolina",
        businessTypeName: bt,
    ...
    assert.equal(
      byId(existing, DOC_HEALTH)?.applicability,
      "needs_more_information",
      `${bt} (${rule}): existing business also sees needs_more_information, not verify_existing`
    );
  }

  // The food path is untouched: a new cafe still gets the sanitary permit
  // as REQUIRED (verified RULE_0058 basis).
  const cafe = classify(
    {
      municipalityName: "San Juan",
      ...
      businessStatus: "new",
      entityNotFormed: true,
      answers: { Q_FOOD_PREPARED: true, Q_PHYSICAL_LOCATION: true },
    },
    "new"
  ).classified;
  assert.equal(
    ...
    "required",
    "a new cafe still gets the food-sanitary permit as REQUIRED"
  );

  // The healthcare demotion is not broken: a new dental office stays NMI.
  const dental = classify(
    {
      municipalityName: "Ponce",
      businessTypeName: "Dental Office",
      businessStatus: "new",
      entityNotFormed: true,
      answers: { Q_PHYSICAL_LOCATION: true, Q_ALCOHOL_SOLD: false },
    },
  ...
  assert.equal(
    byId(dental, DOC_HEALTH)?.applicability,
    "needs_more_information",
    "CASE P demotion still holds for dental offices"
  );
});
