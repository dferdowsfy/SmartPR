...
check("payment: SmartPR charges nothing and never sees the card", /None/.test(handoff) && /never sees your card/.test(handoff));
check("payment: SmartPR credit / saved methods declared unusable here", /can't be used for this government fee/.test(handoff));
